    ...
    def get_short_info(self):
        ...
        for animal in animal_types:
            if self.animals[animal] > 1:
                animal_info.append(animal + "s")
            else:
                animal_info.append(animal)
        
        return f"{self.farm_name}'s farm has {', '.join(animal_info)}."

ronald = Farm("McDonald")
ronald.add_animal('cow', 5)
ronald.add_animal('yew')
ronald.add_animal('horse')
ronald.add_animal('goat', 12)

print(ronald.get_info())

print(ronald.get_short_info())